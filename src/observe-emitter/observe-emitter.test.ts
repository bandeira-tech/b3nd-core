...
Deno.test("ObserveEmitter - multiple concurrent observers both receive", async () => {
  ...
  const b: string[] = [];
  const p1 = (async () => {
    for await (const r of bus.observe(["mutable://app/*"], ac1.signal)) {
      a.push(...r);
      ac1.abort();
    }
  })();
  const p2 = (async () => {
    for await (const r of bus.observe(["mutable://app/*"], ac2.signal)) {
      b.push(...r);
      ac2.abort();
    }
  })();
...
Deno.test("ObserveEmitter - a throwing listener does not break other observers", async () => {
  ...
  const seen: string[] = [];
  const done = (async () => {
    for await (const r of bus.observe(["mutable://x/*"], ac.signal)) {
      seen.push(...r);
...
Deno.test("ObserveEmitter - buffers events emitted before consumer loops back", async () => {
  ...
  const seen: string[] = [];
  const done = (async () => {
    for await (const r of bus.observe(["k/*"], ac.signal)) {
      seen.push(...r);
...
Deno.test("ObserveEmitter - 100 rapid emits arrive in order", async () => {
  ...
  const seen: string[] = [];
  const done = (async () => {
    for await (const r of bus.observe(["stress/*"], ac.signal)) {
      seen.push(...r);
...
Deno.test("ObserveEmitter - buffered events drain before abort terminates the iterator", async () => {
  ...
  const seen: string[] = [];
  const done = (async () => {
    for await (const r of bus.observe(["q/*"], ac.signal)) {
      seen.push(...r);
...
Deno.test("ObserveEmitter - 1000 create+abort cycles leave no listeners registered", async () => {
  ...
  for (let i = 0; i < 1000; i++) {
    const ac = new AbortController();
    const it = bus.observe(["leak/*"], ac.signal)[Symbol.asyncIterator]();
